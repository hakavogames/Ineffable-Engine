...
public class Editor implements GameMode,InputProcessor {
    ...
    public void init(Engine engine) {
        this.engine=engine;
        engine.camera.setToOrtho(false,(float)Gdx.graphics.getWidth()/Gdx.graphics.getHeight()*225,225);
    ...
    public void initUI()
    {
        ...
        savets=new TextButton("Save",skin);
        savets.addListener(new ClickListener() {
            @Override
            public void clicked(InputEvent event,float x,float y)
            {
                map.tileset.save(Gdx.files.local(tsname.getText()));
            }
        });
    ...
    int i;
    Tile t;
    private class TextFieldEvent implements TextField.TextFieldListener {
        public int tile;
        public TextFieldEvent(int id) {
            tile=id;
        }
        @Override
        public void keyTyped(TextField tf,char c) {
            tileset.tiles.get(tile).name=tf.getText();
    ...
    public void initTilesDialog()
    {
        ...
        for(i=0;i<map.tileset.tiles.size;i++)
        {
            ...
            TextField textField=new TextField(t.name,skin);
            textField.setTextFieldListener(new TextFieldEvent(i));
            ...
            b.add(textField).left().padLeft(-5);
            b.setName(t.name);
    ...
    public void update(float delta) {
        ...
        if(Gdx.input.isKeyJustPressed(Input.Keys.C)) {
            tile.collide=(tile.collide != true);
        }
    ...
    public void renderGui(OrthographicCamera cam) {
        ...
        font.draw(GameServices.getSpriteBatch(),
            "Position: "+(int)selStart.x/tileset.tilesize+" x "+(int)selStart.y/tileset.tilesize+
            " Zoom out: "+Math.round(engine.camera.zoom*10)/10f+
            "x  Tile size: "+tileset.tilesize+"  Tile pos: "+tile.tileX+" x "+tile.tileY+
            "  tile.collide:"+tile.collide,10,30);
